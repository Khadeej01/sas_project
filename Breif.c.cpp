...
#define max 50
   ...
   typedef struct tache{
    ...
    int priorite; // plus importante ou normale
    DateEcheance date;
   }Tache; 
   ...
   getchar(); // pour nettoyer le buffer d'entrer 
 ...
 // Affichage des taches 
   ...
   void ModifierTache(){
   	int choix;
   if(nbretaches>0){
   	printf("Donner le numero de tache a modifier(1 a %d)\n",nbretaches);
   	scanf("%d",&numeroTache);
   	if(numeroTache>0 && numeroTache<=nbretaches){
   	 	 getchar();
   	printf("Entrer un nouveau titre\n ");
   	//scanf("%s",T[numeroTache-1].titre); 
   	  fgets(T[numeroTache-1].titre,sizeof(T[numeroTache-1].titre),stdin);
   	 
   	 printf("Entrer une nouvelle description \n");

   	// scanf("%s",T[numeroTache-1].description);
    fgets(T[numeroTache-1].description,sizeof(T[numeroTache-1].description),stdin);
		...
		printf("Entrer la nouvelle annee (actuel: %d) : ", T[numeroTache-1].date.ann);
...
void FiltrerTache() { 
    ...
    for ( i = 0; i < nbretaches; i++) {
        ...
        if (T[i].priorite == priorite) {
            printf("Tache avec priorite %s :\n", priorite== 1 ? "Haute" : "Basse");// OPETAOR TERNAIRE REALISE IF ELSE ON YNE SEULE LIGNE
            printf("Titre : %s\n",T[i].titre);
              // Pour nettoyer le buffer d'entree
            printf("Description : %s\n",T[i].description);
